    ...
    public function validateAppleConfig(?array $config): void
    {
        if (empty($config)) {
            throw new InvalidAppleCredentialsException(
                ...
                . 'Publish the config and set APPLE_CLIENT_ID and APPLE_CLIENT_SECRET in your .env file.',
                ['config' => null],
            );
        }
        ...
        if (! empty($missing)) {
            $envKeys = array_map(
                fn (string $key) => 'APPLE_' . strtoupper($key),
                $missing,
            );
